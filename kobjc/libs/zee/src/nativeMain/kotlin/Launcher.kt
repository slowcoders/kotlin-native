        ...
        args.filter {
            ...
            when (arg.size) {
                1 -> when (arg[0]) {
                    "--gtest_list_tests",
                    "--ktest_list_tests" -> {
                        println("logger.logTestList(this, filterSuites())"); runTests = false
                    }
                    "-h",
                    "--help" -> {
                        println("logger.log(help)"); runTests = false
                    }
...
//        }
    ...
    private fun setLoggerFromArg(logger: String) {
        when (logger.toUpperCase()) {
            "GTEST" -> println("this.logger = GTestLogger()")
            "TEAMCITY" -> println("this.logger = TeamCityLogger()")
            "SIMPLE" -> println("this.logger = SimpleTestLogger()")
            "SILENT" -> println("this.logger = SilentTestLogger()")
...
fun runTestRunner() {
    val args = arrayOf("--ktest_filter=AbstractCollecion.")
    TestRunner(null, args);
}
